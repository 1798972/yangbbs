...
//        RedisProvider.template = template;
//    }
    ...
    //点赞1 未点赞0
    //EditBy Yiang37
    //进行SETBIT 之前，该offset位的比特值。
    public Boolean setBit(String key, Long offect, boolean flag) {
        try {
            Boolean setBit = template.opsForValue().setBit(key, offect, flag);
            return setBit;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public Boolean getBit(String key, Integer index) {
        ...
        return template.execute((RedisCallback<Boolean>) con -> con.getBit(key.getBytes(), index));
        }catch (Exception e){
            e.printStackTrace();
            return false;
        }
    ...
    public Long bitCount(String key) {
        try {
            return template.execute((RedisCallback<Long>) con -> con.bitCount(key.getBytes()));
        }catch (Exception e) {
            e.printStackTrace();
            return -1L;
        }
    ...
    public Long bitCount(String key, int start, int end) {
        ...
        return template.execute((RedisCallback<Long>) con -> con.bitCount(key.getBytes(), start, end));
        }catch (Exception e){
            e.printStackTrace();
            return -1L;
        }
